...
def load_agg(path: str): # Converted Path to string for safer Streamlit caching
    p = Path(path)
    if not p.exists():
        return pd.DataFrame()
    try:
        ...
        st.error(f"Erro ao carregar {p.name}: {e}")
        return pd.DataFrame()

@st.cache_data(ttl=3600)
def load_adverts(path: str):
    p = Path(path)
    if not p.exists():
        return pd.DataFrame()
    try:
...
# LOAD DATA
# -------------------------
# Pass as strings to avoid Streamlit unhashable Path errors
agg = load_agg(str(AGG_FILE))
adverts = load_adverts(str(ADVERTS_FILE))

# -------------------------
# CHECK DATA
# -------------------------
if agg.empty:
    st.warning("Arquivo de base agregada (cards_daily_aggregated.parquet) não encontrado ou vazio. Rode o processamento incremental primeiro.")
    st.stop()
...
# Handle case where user only selects one date in the range picker
if len(date_range) > 1:
...
avg_volatility = float(df['std_price'].mean(skipna=True)) if not df.empty else 0.0
avg_turnover = float(df['turnover_rate'].mean(skipna=True)) if not df.empty else 0.0
...
if df_card.empty:
    ...
    if 'rolling_avg_price' in df_card.columns:
        fig.add_trace(go.Scatter(x=df_card['date_collected'], y=df_card['rolling_avg_price'], mode='lines', name='Rolling Avg (7d)', line=dict(dash='dash')))
    fig.update_layout(title=f"Tendência de preço — {card_for_detail or (top_card_name if top_card_name else '')}", xaxis_title="Data", yaxis_title="Preço (zeny)", legend=dict(orientation="h"))
